...
namespace Grace.Tests.ThridParty.Optional
{
    public class OptionalStrategy : SimpleGenericStrategy
    {
        ...
        {
            _optionNone = typeof(Option).GetTypeInfo().DeclaredMethods
                .First(m => m.Name == "None" && m.GetParameters().Length == 0);
            _optionSome = typeof(Option).GetTypeInfo().DeclaredMethods
